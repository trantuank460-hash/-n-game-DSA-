...
def play_background_music():
    try:
        pygame.mixer.music.load("music_bg.mp3") # Tải file nhạc nền
        pygame.mixer.music.set_volume(0.5)      # Chỉnh âm lượng (0.0 đến 1.0)
        pygame.mixer.music.play(-1)             # Tham số -1 để nhạc lặp vô tận
...
# Khởi động nhạc nền trước khi vào vòng lặp
...
while running:
    ...
    for hit in hits:
        snd_explosion_small.play() # Sử dụng hiệu ứng nổ nhỏ
        score += 10
        enemies_killed += 1
    ...
    for boss_hit in boss_hits:
        ...
        score += 5
        if boss_hit.hp <= 0:
            snd_explosion_boss.play() # Sử dụng hiệu ứng nổ boss (lớn)
            boss_hit.kill()
            score += 1000 * level
    ...
    if pygame.sprite.spritecollide(player, enemy_bullets, True):
        player.hp -= 1
        snd_player_hit.play() # Sử dụng hiệu ứng nổ cho người chơi
